# DeskPulse 0.3.3.2 Technical Development Handover
...
DeskPulse **0.3.3.2** locks in the tested tray-menu, Windows 11 flyout, Excel-export progress and tray-state icon fixes while retaining the architecture released in 0.3.3.0.
...
- **DeskPulse updated** when the detected prior version differs from 0.3.3.2.
- **DeskPulse reinstalled** when 0.3.3.2 is installed over the same version.
...
Start-Process ".\publish\v0.3.3.2\installer\DeskPulse_Setup_0.3.3.2.exe"
```
...
2. Publish outputs exist under `publish\v0.3.3.2\service` and `publish\v0.3.3.2\tray`.
3. The installer is created as `DeskPulse_Setup_0.3.3.2.exe`.
4. Installer upgrades the accepted 0.3.0.0 or 0.3.0.1 installation.
5. Service and tray report version 0.3.3.2.
...
18. GitHub-facing README, CHANGELOG, release notes and handovers identify 0.3.3.2 as current.
...
The 0.3.2.x storage and security acceptance was completed on 2026-07-23. Final 0.3.3.2 build, packaging and installation results are recorded in the repository `VERSION_CHECK.md`.
